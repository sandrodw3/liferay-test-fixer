...
description: Run the project's `collect` script for a given Testray routine ID, producing a JSON snapshot of that routine's most recent failures at `output/test-failures-<routineId>-<YYYY-MM-DD>.json`. The snapshot includes per-failure metadata (name, type, `errorTrace`, `lastPassSha`, `firstFailSha`) and is the input expected by `/analyze-routine-failures`. Use this skill whenever the user wants to refresh the snapshot for a routine before analysing it.
---

# Collect routine failures

Produce a fresh JSON snapshot of a Testray routine's recent failures by running the project's `collect` script. This skill is the data-gathering half of the failure-analysis flow — `/analyze-routine-failures` then operates on the snapshot this skill writes.
...
- Do **not** run any analysis on the JSON. That is `/analyze-routine-failures`.
